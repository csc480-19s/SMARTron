...
class GenericDaoTest extends Specification {//is this even going to be used or did I just waste my time?...
    ...
    def "Insert"() {//3.5.2
        ...
        def beforeList = genDao.select("SELECT * FROM scantron.exam WHERE first_name='Kevin';")//either i cant write SQL statements for shit, or this broken
        genDao.insert("INSERT INTO scantron.exam ("+
                ...
                ");")
        def afterList = genDao.select("SELECT * FROM scantron.exam where first_name='Kevin';")
    ...
    def "Delete"() {//3.5.4
        ...
        def beforeList = genDao.select("SELECT instructor_id FROM scantron.instructor")//this scans from NOT instructor... broke?
        genDao.delete("DELETE FROM scantron.instructor WHERE instructor_id='64535'")
        def afterList = genDao.select("SELECT instructor_id FROM scantron.instructor")
        then: "entry deleted"
        noExceptionThrown()
        beforeList.size() == 1
        afterList.size() == 0
