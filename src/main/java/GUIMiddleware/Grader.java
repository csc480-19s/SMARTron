    ...
    public List<Float> getGrades(List<Student> studentExams, Student key){
        ...
        for (Student studentExam : studentExams) {
            ...
            for (int j = 0; j < key.getAnswers().size(); j++) {
                if (studentExam.getAnswers().size() > j) {if (studentExam.getAnswers().get(j) != null && key.getAnswers().get(j).contains(studentExam.getAnswers().get(j)) && !key.getAnswers().get(j).contains("-1")) {                    //Compares the jth  position in the answer array from ith studentExam to the key
