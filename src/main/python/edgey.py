        ...
        for co, c in zip(col, centers):
            if c not in visited and c not in done:
                ...
                for coo,cc in zip(col, centers):
                    if cc not in tempvisit and cc not in done:
                        if abs(c[1] - cc[1]) < (imgH*0.2667995): #0.2667995
                            temp.append(coo)
                            tempvisit.append(cc)
                if (len(temp) == 27):
                    for x in tempvisit:
...
dark1 = cv2.dilate(dark, kernel, iterations=1)
...
edges2 = cv2.morphologyEx(edges, cv2.MORPH_CLOSE, np.array([[-1,-1,-1], [-1,9,-1], [-1,-1,-1]]))
#sharp1 = cv2.morphologyEx(sharp1, cv2.MORPH_ERODE, kernel)
...
co1 = cv2.findContours(dark1, cv2.RETR_LIST, cv2.CHAIN_APPROX_SIMPLE)[0]
...
contours = np.append(contours, co1, axis=0)
contours = np.append(contours, contours2, axis=0)
